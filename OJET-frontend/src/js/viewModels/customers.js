  ...
  function (accUtils, ko) {
    function CustomerViewModel() {
      ...
      self.totalCost = ko.computed(() =>
        self.stocks().reduce((sum, stock) => sum + stock.quantity * stock.listingPrice, 0).toFixed(2)
      );

      self.profitPercent = ko.computed(() => {
        const cost = self.totalCost();
        const value = self.totalHoldings();
        return cost === 0 ? 0 : (((value - cost) / cost) * 100).toFixed(2);
      });

      self.profitColor = ko.computed(() =>
        self.profitPercent() >= 0 ? 'grey' : 'grey'
      );
